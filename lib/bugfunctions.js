...
async function sendCrashMessage(sock, jid, type = 'xeontext8') {
  try {
    const crashText = bugTexts[type] || bugTexts.xeontext8
...
async function sendRepeatedCrash(sock, jid, count = 5, type = 'xeontext8') {
  try {
...
async function sendIOSCrash(sock, jid) {
  return sendCrashMessage(sock, jid, 'xeontext8')
}

// Send system crash
async function sendSystemCrash(sock, jid) {
  return sendCrashMessage(sock, jid, 'xeontext7')
}
